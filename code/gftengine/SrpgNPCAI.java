...
public class SrpgNPCAI {

    public final ActionRoad searchPath(SrpgMap srpgMapSystem,int startX,int startY,int endX,int endY){
        Direction direction = Direction.none;
        ActionRoad actionRoad = new ActionRoad();
        actionRoad.setNextNode(null);
        actionRoad.setNextBlock(direction);
        MapBlock map[][]=srpgMapSystem.getMap();
        return actionRoad;
    }
    ...
    public void startAction(GFTEngine context, AIEvent aiEvent) {
        //传入AI角色信息并开始规划行动
        //声明
        SrpgDatabaseSystem srpgDatabaseSystem = context.getSrpgDatabaseSystem();
        int ax = aiEvent.getActorInfo().getX(), ay=aiEvent.getActorInfo().getY(), bx = 0, by = 0;
        Role tempRole;
        //开始执行逻辑
        searchPath(context.getSrpgMapSystem().getActiveMap(), ax, ay, bx, by);
    };
